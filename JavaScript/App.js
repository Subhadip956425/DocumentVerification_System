//IPFS api key in ifura.io
const projectId = "28LuNAotbXzcvtpOcE9F8ayKOeP";
//api secret in ifura.io
const projectSecret = "3de3d9c099c6c0c168e39b8bc03e2f7a";
window.CONTRACT = {
  address: "0x61Ea3fbDe50a30A41529B068948a7a5F44BaAf61",
...
window.onload = async () => {
  ...
  if (window.ethereum) {
    ...
    if (window.userAddress.length > 10) {
      ...
      if (window.location.pathname == "/Pages/Admin.html") await getCounters();
...
async function verify_Hash() {
  ...
  if (window.hashedfile) {
    try {
      // Call the contract to find the document hash
      ...
      // Check if the result indicates a valid document
      ...
      print_verification_info([], false);
    }
  } else {
    // Hide the loader if no file is selected
    $("#loader").hide();
...
async function sendHash() {
  try {
    ...
    // Call get_ChainID if needed
    get_ChainID();

    // Check if the file hash already exists on the blockchain
    if (window.hashedfile && window.hashedfile.length > 4) {
      const existingRecord = await window.contract.methods
        .findDocHash(window.hashedfile)
        .call({ from: window.userAddress });

      // If the document already exists on the blockchain, show an error and stop further processing
      if (existingRecord[0] != 0 || existingRecord[1] != 0) {
        throw new Error("The document already exists on the blockchain.");
      }
      ...
      // Interact with the smart contract to add the document hash and CID
      await window.contract.methods
        .addDocHash(window.hashedfile, CID)
        .send({ from: window.userAddress })
        .on("transactionHash", function (_hash) {
          $("#note").html(
            `<h5 class="text-info p-1 text-center">Please wait for the transaction to be mined...</h5>`
          );
        })
        ...
        .on("confirmation", function (confirmationNr) {
          console.log("Transaction confirmation number:", confirmationNr);
        })
      ...
      console.error("Invalid file hash:", window.hashedfile);
      $("#note").html(
        `<h5 class="text-center text-warning">Invalid file hash detected 😕</h5>`
      );
    }
  } catch (error) {
    console.error("Error in sendHash function:", error.message);
    $("#note").html(`<h5 class="text-center">${error.message} 😏</h5>`);
    $("#loader").addClass("d-none");
    $("#upload_file_button").slideDown();
  }
...
async function updateRecentDocuments() {
  try {
    ...
    // Fetch recent documents from the blockchain (assuming you have a function for this)
    const recentDocuments = await fetchRecentDocumentsFromBlockchain(); // Implement this function based on your contract
        ...
        <div class="col-12 col-md-6 col-lg-4">
          <div class="card mb-3">
            <div class="card-body">
              ...
              <a href="${doc.link}" class="btn btn-primary">View Document</a>
            </div>
          </div>
        </div>
      `);
...
async function deleteHash() {
  ...
  try {
    if (!window.hashedfile) {
      throw new Error("No hash provided for deletion.");
    }
    ...
    if (result[0] === "0" && result[1] === "0") {
      throw new Error("The document does not exist on the blockchain.");
    }

    // Proceed with deletion if the document exists
    await window.contract.methods
      ...
      .on("receipt", function (receipt) {
        ...
        // Update recent documents section
...
//hash a document using the SHA-3 hashing algorithm provided by the Web3.js library
function get_Sha3() {
  hide_txInfo();
  $("#note").html(`<h5 class="text-warning">Hashing Your Document 😴...</h5>`);
...
async function addExporter() {
  ...
  if (info && address) {
    ...
    try {
      ...
      if (window.userAddress.toLowerCase() !== ownerAddress.toLowerCase()) {
        throw new Error("You are not authorized to add an exporter.");
      }
...
async function editExporter() {
  ...
  if (info && address) {
    ...
    try {
      ...
      // Check if the current user is the owner
      if (window.userAddress.toLowerCase() !== ownerAddress.toLowerCase()) {
        throw new Error("You are not authorized to edit an exporter.");
      }
...
async function deleteExporter() {
  ...
  if (address) {
    ...
    try {
      ...
      // Check if the current user is the owner
      if (window.userAddress.toLowerCase() !== ownerAddress.toLowerCase()) {
        throw new Error("You are not authorized to delete an exporter.");
      }

      // Check if the address exists
      const exporterInfo = await window.contract.methods
        .getExporterInfo(address)
        .call();

      if (!exporterInfo || exporterInfo === "") {
        throw new Error("The address does not exist on the blockchain.");
      }
...
//note: in case of using local server need to replace 127.0.0.1 with your machine local ip address got from the router
function generateQRCode() {
  // Clear the previous QR code
  document.getElementById("qrcode").innerHTML = "";
  console.log("making qr-code...");

  // Ensure QRCode library is included in your project
  ...
  // Check if hashedfile is available
  if (!window.hashedfile) {
    console.error('Hashed file not found.');
    return;
  }

  // Create the URL with the document hash
  let url = `${window.location.origin}/Pages/Verify.html?hash=${window.hashedfile}`;
  qrcode.makeCode(url);

  // Set up the download and verification links
  var docFileInput = document.getElementById("doc-file");
  if (docFileInput && docFileInput.files.length > 0) {
    document.getElementById("download-link").download = docFileInput.files[0].name;
  }

  document.getElementById("verfiy").href = url;

  // Ensure QR code is fully rendered before setting the download link
  function makeDownload() {
    var qrImage = document.querySelector("#qrcode img");
    if (qrImage) {
      document.getElementById("download-link").href = qrImage.src;
    } else {
      console.error('QR code image not found.');
    }
  }

  // Wait a bit to ensure QR code rendering is complete
...
async function listen() {
  console.log("started...");
  if (window.location.pathname != "/Pages/Upload.html") return;
